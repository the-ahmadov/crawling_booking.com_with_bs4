...
def scrape_room_info(url, hotel_id):
    ...
    for row in rows:
        ...
        roomtype = row.find("a",attrs={"class":"jqrt"})
        bed = row.find("td",attrs={"class":"ftd roomType"})
        roominfo = row.find("div", attrs={"class":"room-info"})
        ...
        if sleeps:
            ...
            # try:
            #     if roomtype:
            #         roomid = check_empty(roomtype['href'][3:])
            ...
            temp_sleeps = check_empty(sleeps)
            temp_roomtype = check_empty(roomtype)
            temp_bedtype = s.strip()
            temp_roomid = roominfo['id'][3:]

            room_detail_list = [temp_roomid,temp_sleeps,temp_roomtype,temp_bedtype,"No size yet",hotel_id]
...
def scrape_hotel_info(url,hotel_id):
    ...
    property_type = check_empty(hotel_name_tag.find("span"))
    ...
    try:
        ...
        while True:
            ...
            for hotel in hotels:
                ...
                hotel_count+=1
            list_cur_len = len(hotel_id_list)
            print("Hotel Count = ", hotel_count," list len-> ", len(hotel_id_list))
            wb.save("booking_scrape.xls")
            #Check if page exists
            page_offset+=25
            # if hotel_count >= found_props:
            if list_cur_len != list_prev_len:
                list_prev_len = list_cur_len
            else:
                break
        # Save in excel file
        wb.save("booking_scrape.xls")
